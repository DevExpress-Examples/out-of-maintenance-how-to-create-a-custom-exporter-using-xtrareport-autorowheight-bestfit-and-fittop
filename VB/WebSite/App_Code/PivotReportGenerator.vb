...
Public NotInheritable Class PivotReportGenerator
	...
	Private Shared Sub FillDatasetColumns(ByVal pivot As ASPxPivotGrid, ByVal dataTable1 As DataTable)
		...
		Dim tempColumnText As String = ""
		Dim fieldsInColumnArea As List(Of PivotGridFieldBase) = GetFieldsInArea(pivot, PivotArea.ColumnArea)

		For i As Integer = 0 To pivot.ColumnCount - 1
			Dim pcea As DevExpress.XtraPivotGrid.Data.PivotGridCellItem = GetCellItem(pivot, i, 0)
			If pcea.ColumnValueType = PivotGridValueType.Value Then
				For Each field As PivotGridFieldBase In fieldsInColumnArea
					tempColumnText &= pcea.GetFieldValue(field).ToString() & " | " 'add formatting if it's necessary
				Next field
				tempColumnText = tempColumnText.Remove(tempColumnText.Length - 3, 3)
				dataTable1.Columns.Add(tempColumnText, GetType(Object))
				tempColumnText = ""
			Else
				dataTable1.Columns.Add(pcea.ColumnValueType.ToString(), GetType(Object))
			End If
		Next i
	End Sub
	...
	End Function
	 Private Shared Sub InitDetailsBasedonXRTableWithoutRepeatingRowHeader(ByVal rep As XtraReport, ByVal kind As ReportGeneratorType, ByVal columnWidth As Integer)
		...
		For i As Integer = 0 To colCount - 1
			If kind = ReportGeneratorType.BestFitColumns Then
				If i = 0 Then
					AddCellsToTables(tableHeader, tableDetail, ds.Tables(0).Columns(i),columnsWidth(i),True)
				Else
					AddCellsToTables(tableHeader, tableDetail, ds.Tables(0).Columns(i),columnsWidth(i),False)
				End If
			Else
				If i = 0 Then
					AddCellsToTables(tableHeader, tableDetail, ds.Tables(0).Columns(i),colWidth,True)
				Else
					AddCellsToTables(tableHeader, tableDetail, ds.Tables(0).Columns(i),colWidth,False)
				End If
			End If
		Next i
		tableDetail.EndInit()
